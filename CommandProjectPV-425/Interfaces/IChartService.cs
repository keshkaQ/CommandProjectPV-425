...
namespace CommandProjectPV_425.Interfaces
{
    ...
    public interface IChartService
    {
        ...
        (ISeries[] Series, Axis[] XAxes, Axis[] YAxes) CreateColumnChart(List<string> labels,
            List<double> values,
            string xAxisName,
            string yAxisName,
            Func<double, string> yLabelFormatter,
            Func<int, double, string> tooltipFormatter, bool isSpeedupChart = false);

        (Axis[] X, Axis[] Y) CreateAxes( string xName, string yName, List<double> values,Func<double, string> yLabelFormatter, bool isSpeedupChart = false);
    }
}
